...
- [ ] **main マージ時の auto-deploy (CD) 設定**
  - 現状: `flyctl deploy` 手動。 main の最新と本番に 19 時間ギャップが空くケースもあり、 環境依存の壊れ方 (env var typo / fly config drift / migration 失敗) の発見が遅れる
  - 方針: 概ね GO（個人プロダクトサイズ、 CI ＋ branch protection で既にゲート済み、 PR #13 の CancellationToken で SIGTERM ローリング再起動が安全になった）
  - 残懸念:
    1. **視覚回帰の盲目化** — Playwright 等の自動視覚回帰が無い間、 UI 変更の PR がデザイン崩れたまま本番に出る (PR #16 が browser 未確認で merge した実例)。 視覚回帰テスト導入を CD の前提条件にするかどうか
    2. **migrations の手動ゲート** — SQLite ALTER TABLE は不可逆系もあり、 PR レビュー段階の見落としが直撃する。 `migrations/**` を含む PR では GitHub Environments の manual approval を挟むかどうか
  - 実装スケッチ: `.github/workflows/deploy.yml` で `push: { branches: [main] }` トリガ、 `superfly/flyctl-actions/setup-flyctl@master` → `flyctl deploy --remote-only`、 secret `FLY_API_TOKEN` 設定。 docs / README のみの merge では `paths-ignore: ['docs/**', 'README.md']` で skip して無駄を抑える

- [ ] **視覚回帰テスト** — CD 入れる前に整える優先度に上げ。 候補: Playwright screenshot diff を CI に組み込み、 home / 空状態の 2 ケースだけでも押さえる。 ZINE redesign 由来 (Tailwind v4 / 手書きブレイクポイント) の崩れと、 album_name / spotify_image_url の有無での描画差分を catch する用途
