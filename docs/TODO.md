...
- [ ] **視覚回帰テスト** — CD 化が先行したため優先度上昇。 候補: Playwright screenshot diff を CI に組み込み、 home / 空状態の 2 ケースだけでも押さえる。 ZINE redesign 由来 (Tailwind v4 / 手書きブレイクポイント) の崩れと、 album_name / spotify_image_url の有無での描画差分を catch する用途
