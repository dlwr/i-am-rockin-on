...
- [ ] **Spotify Search のクエリエスケープ**（過去 TODO の昇格）
  - `format!("artist:\"{}\" album:\"{}\"", ...)` で `"` `\` 混入時に silent miss。 PR #2 以降は silent miss → 行が 0 件になるため UX 影響が直撃する（旧挙動はマッチ無しでも保存しとった）
  - 場所: `src/server/resolver/spotify.rs:69-83`（現在の resolve 内）
...
- [ ] **`main` を保護ブランチ化**
  - PR #3 で CI が main の "Required check" 候補に入った。あとは GitHub UI の Settings → Branches で `main` に対して「Require a pull request before merging」「Require status checks to pass」（`ci` を必須）を有効化するだけ。CLI 作業ではない／不可逆寄り（PR 経由が必須になる）ので user 判断
  - 場所: GitHub Settings → Branches → Branch protection rules

- [ ] **CI 上の cargo-leptos / sqlx-cli インストールキャッシュ**
  - 初回 7 分かかったうち過半は `cargo install sqlx-cli` と cargo-leptos prebuilt 取得。`Swatinem/rust-cache` は target/ をキャッシュするが `~/.cargo/bin` は対象外
  - 対処: `actions/cache` で `~/.cargo/bin/{sqlx,cargo-leptos}` をバージョン pin した key で別途キャッシュ、または cargo-leptos は GitHub Releases binary のチェックサム＋ key にしてダウンロードを skip
