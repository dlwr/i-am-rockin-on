...
- [ ] **config テストの env 汚染による稀な flake 解消**
  - `src/server/config.rs` の `missing_database_url_errors` が、`from_env` 系テストとの並列実行で `DATABASE_URL` のプロセス env を奪い合い、高並列時に稀に失敗する（PR #36 の作業中に観測。3回再実行では再現せず、頻度は低い）
  - 原因: config テストが `std::env::set_var` / `remove_var` でプロセスグローバルな env を変更しており、並列実行でレースする
  - 対処案: `serial_test` クレート導入、または env を変更するテスト群を共有 Mutex でシリアライズ
  - 優先度: 低（既存の latent な問題。本フィーチャー起因ではない）
...
- [x] **RSS から落ちた古い「推し」のバックフィル** — 案A を採用・実装。rokinon を RSS（最新10件）から entrylist ページネーション（`ROKINON_MAX_PAGES`、既定5）+ 各記事のフルページ fetch に切り替え。`scraped_entries` テーブルで記事ごとに処理済みを追跡し、パイプラインに fetch 前スキップを追加（全ソース共通）。初回は entrylist を遡って取りこぼし分を回収、2回目以降は未処理の新着のみ fetch する自己修復型。マイグレーションで既存 recommendations から bootstrap し初回 re-fetch 嵐を防止。transient 失敗（HTTP error・Spotify 解決失敗）では mark せず次回リトライを温存。抽出は `#entryBody` スコープ + og:title から artist（`『』` 除去）。発端は `entry-12966301740`（2026年5月推し）の取りこぼし（PR #36）
  - 設計: `docs/superpowers/specs/2026-05-25-rokinon-entrylist-backfill-design.md` / 計画: `docs/superpowers/plans/2026-05-25-rokinon-entrylist-backfill.md`
  - 採らなかった案: 案B（週次全件再フェッチ）/ 案C（月次アーカイブ走査）。entrylist 走査 + seen-set で恒常的に解消したため不要
  - hotfix (PR #38): 実 ameblo の entrylist は相対 href (`/stamedba/entry-….html`) を返すが `list_candidates` が href をそのまま `source_url` にしていたため `client.get(相対パス)` が builder error で全件失敗し、デプロイ以降 rokinon の取り込みが全滅していた。`make_absolute` で base_url を前置して解決。テストのフィクスチャが絶対 URL を使っていて実態と乖離していたのが見逃しの原因 → フィクスチャを相対 href に修正し、`source_url` が絶対 URL であること + list→fetch を通す end-to-end 回帰テストを追加。教訓: フィクスチャは実データのフォーマット（相対/絶対）に合わせる
  - 並び順修正 (PR #40): rokinon の `featured_at` が「YYYYMM推し」を月初 (`from_ymd_opt(y, m, 1)`) に丸めていたため、同月の記事が全て同一日付になり、`ORDER BY featured_at DESC, dedup_key ASC` の月内順序が dedup_key（spotify_url／なければ artist|album）の辞書順という時系列無関係なタイブレーカーで決まっていた。GIVĒON「BELOVED: ACT II」(実 5/16) が Francis of Delirium「Run, Run Pure Beauty」(実 5/29) より古いのに上に出る症状で発覚。JSON-LD (`@type:BlogPosting`) の `datePublished` を抽出し `date_naive()`（JST `+09:00` を保持。`naive_utc().date()` だと UTC 変換で 1 日前にずれる）で `featured_at` に使うよう変更。取得不可時は推し月初へフォールバック（`tracing::warn`）、「YYYYMM推し」判定は妥当性ゲートとして維持。本番は `scraped_entries` の rokinon 分を削除（120 件）→ `/app/scrape --source rokinon` 再走査でバックフィル（added 1 / updated 19 / skipped 80）、本番サイトで Francis (上) / GIVĒON (下) の入れ替わりを確認済み。教訓: 月単位に丸めた日付をソートキーにすると、月内の時系列がタイブレーカー（dedup_key 等）に化けて新旧と無関係になる

- [x] **他メディアソース追加（Pitchfork）** — `src/server/adapter/pitchfork.rs` で 8.0+ かつ直近 90 日を取得。同一アルバムは Rokinon と統合表示

- [x] **他メディアソース追加（funkstudy / X の taizooo）** — `src/server/adapter/funkstudy.rs` で X の taizooo の `#yetanotherfunkstudy` 付きポストを twitterapi.io 経由で取得し、ぶら下がる本人返信の Spotify アルバム URL から取り込む。アルバム URL を直接持つので `SpotifyResolver::resolve_by_album_id`（`GET /v1/albums/{id}`）で解決し、ピプラインは `spotify_url` 事前セット時のみこの ID 解決に分岐（既存ソース不変）。返信がまだ無い本体は `AppError::Retryable` で次回リトライ（検索窓で自然消滅）。`FUNKSTUDY_API_KEY` 未設定なら main はソース未登録（PR #42）
  - 設計: `docs/superpowers/specs/2026-06-01-funkstudy-source-design.md` / 計画: `docs/superpowers/plans/2026-06-01-funkstudy-source.md`
  - 実 API スモークテストで判明し修正した契約のズレ: ①replies レスポンスの top-level は docs の `replies` ではなく実際は `tweets`（alias 対応。無いと常に空のサイレント失敗）②`has_next_page` が空ページでも true を返すためページング廃止・1 ページ取得に ③QPS 制限の 429（`Retry-After` なし）に exponential backoff（3/6/12s）追加。search 直後の replies が必ず 429 になり無いと実質動かなかった。教訓: 外部 API 統合はモックだけで done にせず実接続スモークを通す
  - スモーク結果: taizooo の実投稿が Cyndi Lauper「Memphis Blues」(featured_at=2026-04-09) として DB 挿入されることを確認
  - 本番 enable 済み: `flyctl secrets set FUNKSTUDY_API_KEY=...` を設定し有効化（2026-06-02）。30 日窓を超える初回バックフィルは 1 ページ（最大 20 件）でしか拾えない（低頻度なので実害小）
  - **複数ハッシュタグ対応（PR #44）**: taizooo が最近 `#yetanotherbachstudy` を多用するため funk と両対応に。`FUNKSTUDY_HASHTAGS`（カンマ区切り・`#` 任意・既定 `yetanotherfunkstudy,yetanotherbachstudy`）で増減でき、複数なら advanced_search を `(#a OR #b)` で OR 検索。本体＋返信の Spotify 構造は同一なので抽出ロジックは不変。実 API スモーク済み（bachstudy 4 件: UA / Björk / Paul McCartney / Nuallan が end-to-end 解決）
  - follow-up: 本番は初回スクレイプ実行済み（`log.count>0`）のため、既存の直近 funk/bach は次回 cron（JST 07:00）で反映される。即時反映したい場合は `flyctl ssh console -C "/app/scrape --source funkstudy"` を手動実行（auto-mode では本番 shell write が拒否されるため要手動）
  - **表示ラベル修正（PR #46）**: `funkstudy` が `src/pages/home.rs` の `source_label` に未登録で、記事メニュー（複数ソース時のドロップダウン）に生 `source_id` のまま出ていた。`funkstudy → yetanother(funk|bach)study` を追加し、ついでに `rokinon` を `ロキノン → ロキノンには騙されないぞ`（元ネタの Ameblo タイトル）に変更。`source_label_known_ids` テストも更新。教訓: 新ソース追加時は adapter/config だけでなく `source_label` の表示マッピングも揃える（漏れると未知 id passthrough で生 ID が UI に漏れる）
  - **ハッシュタグ別ラベル ＋ #FUNKStudy 取り込み（PR #50）**: 全部 `source_id="funkstudy"` に畳んで単一ラベル `yetanother(funk|bach)study` 表示だったのを、投稿ごとに実際のハッシュタグ名で出せるようにした。`list_candidates` で検索結果 tweet の text からハッシュタグを検出（`match_configured_hashtag`: `#` アンカー + case-insensitive・設定順で先頭一致・設定側の正準表記に正規化。`#` アンカーで `#funkstudy` が `#yetanotherfunkstudy` に誤マッチしない）し、`CandidateRef.source_id_override` で `fetch_and_extract` へ運んで保存行 `source_id` に使う。**スキーマ変更なし**: scrape 追跡 `is_scraped`/`mark_scraped` はアダプタ `id()="funkstudy"` のまま・保存行 `source_id` とは元から分離している構造を利用。`#FUNKStudy` を既定検索タグに追加（adapter / config 既定 + README、`"FUNKStudy"`(行 source_id) と `"funkstudy"`(追跡 id) は case-sensitive に別物）。`source_label` を invert 方式に変更し、rokinon/pitchfork/旧 funkstudy は明示マッピング・それ以外（funkstudy 系タグ・`FUNKSTUDY_HASHTAGS` で env 追加分含む）は `#tag` フォールバック表示（env 拡張がデプロイ無しで表示にも効く）。表示箇所は従来どおり複数ソースカードのドロップダウン内のみ。制約: デプロイ前にスクレイプ済みの funkstudy 行はタグ未保存のため**バックフィル不可**、旧結合ラベルのまま残りデプロイ後の新規取得分のみ per-hashtag になる。実 API スモーク（ローカル DB）で yetanotherfunkstudy 2 / yetanotherbachstudy 3 / FUNKStudy 1（James Brown「In The Jungle Groove」）が per-hashtag で分離・取り込まれること、旧 funkstudy への fallback 0 件を確認。教訓: ①per-post の分類が要る時は行ごとに `source_id` を変えるのが最小（scrape 追跡 id と保存 source_id が分離している構造を使えばスキーマ追加不要） ②検出は「データが構造上必ず存在する地点」（検索結果 text）で行う。観測挙動依存の地点（返信レスポンスの本体ポスト有無）だと滅多に見ないドロップダウン内で無言フォールバックして気付けない
    - 設計: `docs/superpowers/specs/2026-06-09-funkstudy-hashtag-labels-design.md` / 計画: `docs/superpowers/plans/2026-06-09-funkstudy-hashtag-labels.md`
    - follow-up: 本番は初回スクレイプ済みのため過去 funk/bach 行は旧結合ラベルのまま残る。#FUNKStudy 含む新規は次回 cron（JST 07:00）で反映。即時反映は `flyctl ssh console -C "/app/scrape --source funkstudy"` を手動実行（auto-mode では本番 shell write 拒否のため要手動）
